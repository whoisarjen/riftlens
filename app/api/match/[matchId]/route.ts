...
export async function GET(
  ...
  try {
    ...
    if (matchRows.length > 0) {
      ...
      const detail = transformFromDb(matchRows[0], participantRows);
    ...
    try {
      ...
      console.error(`Failed to fetch timeline for ${matchId}:`, err);
    }

    // 4. Cache in DB
    try {
      await upsertMatch(matchData, region);
      // Also store timeline data if available
      if (timeline) {
        await db
          .update(matches)
      ...
      console.error(`Failed to cache match ${matchId}:`, err);
    }

    // 5. Transform and return
    const detail = transformMatch(matchData, timeline);
    return NextResponse.json(detail);
  } catch (error) {
